...
model_base = AutoModelForVision2Seq.from_pretrained(config.base_model_name_or_path, 
                                                    cache_dir=cache_dir,
                                                    torch_dtype=torch.float16,
                                                    # quantization_config=quantization_config,
                                                    )
...
model_trans_sr_en = AutoTokenizer.from_pretrained(model_trans_sr_en, quantization_config=quantization_config, cache_dir=cache_dir)
tokenizer_sr_en = AutoModelForSeq2SeqLM.from_pretrained(model_trans_sr_en, cache_dir=cache_dir)
...
def chat_engine(chat_history: list,image: Image, max_new_token: int) -> str:
    ...
    if image!= None:
        ...
        start = time.time()
        generated_ids = model_vqa.generate(**inputs, max_new_tokens=max_new_token)
...
def gradio_interface_llm(current_message_gradio: dict, history: any):
    """
    ...
    if len(chat_history) == 0:
        ...
        if no_image == 0:
            ...
            output = return_translation(output, source_lang)
        history = get_template_assistant(output,chat_history)
